...
def do_turn(game):
    ...
    if game.get_turn() < EARLY_TURNS and len(game.get_all_islands()) > 1:
        ...
        game_state = "CONTROL"
    debug(game,game.get_max_drones_count())
...
def handle_pirates(game, game_state, battles):
        ...
        while len(pirates) > 0:
            ...
            # Defend an island if an enemy is close and you can intercept him
                ...
                if drone_move.get_dist() <= 11:
                    ...
                    destination = Location(rows/2, min(cols-1, 10 + game.get_myself().id * 26))
            ...
            # If all else fails, go to the middle of the map so you dont crash
                ...
                destination = Location(rows/2, cols/2)
                sailing = optimize_pirate_moves(game, pirates[0], destination)
                if pirates[0] not in semi_used_pirates:
    ...
    # Rushing with the stack and pirates towards the enemies that are closest to the city
        ...
        stack_location = Location(stack_location.row, min(cols-1, stack_location.col + 2 - 4 * game.get_myself().id))
...
def optimize_drone_moves(sail_options, game):
    ...
    if (sail_options[0].row - rows/2) ** 2 + (sail_options[0].col - cols/2) ** 2 > (sail_options[1].row - rows/2) ** 2 +\
                    (sail_options[1].col - cols/2) ** 2:
        return sail_options[0]
    else:
        return sail_options[1]
...
def optimize_pirate_moves(game, pirate, destination):
    ...
    for option in sail_options:
        ...
        option_value -= math.hypot(option.row - rows/2, option.col - cols/2)
