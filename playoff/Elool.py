...
def handle_drones(game, game_state):
	...
	if game_state == "CONTROL":
		...
		for plan in drones_plans:
			game.debug(plan['id'])
			game.debug(plan['steps'])
			game.debug(game.get_my_drone_by_id(plan["id"]))
			game.debug('*****')
			if plan["steps"] != [] and plan["id"] in living_drones_ids:
				drone = game.get_my_drone_by_id(plan["id"])
...
def target_city(game, stack_location):
	...
	if not game.get_my_dome() and game.get_my_defense_points() < game.get_required_defense_points() and game.get_enterprise().value_multiplier in [3,1]:
