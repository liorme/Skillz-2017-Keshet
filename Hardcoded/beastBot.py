    ...
    if game.get_turn() < EARLY_TURNS:
        game_state = "EARLY"
          ...
          or game.get_max_turns() - game.get_turn() < MAX_TIME_TO_RUSH) or (
                    ...
                    game.get_my_living_drones()) > MIN_DRONES_ALIVE_TO_CONTINUE_RUSH or game.get_max_turns() - game.get_turn() < MAX_TIME_TO_RUSH):
        game_state = "RUSH"
...
def handle_pirates(game, game_state, battles):
    ...
    # Try to get islands, kill drones, kill pirates, and gain map control in general
        ...
        while len(pirates) > 0:
            ...
            elif len(islands) > 0 and len(enemy_drones) > 0:

                move_island = best_move(pirates, islands)
                move_drone = best_move(pirates, enemy_drones)

                # choose better move
                dist = lambda x: x.get_aircraft().distance(x.get_location())
                if (dist(move_drone) - 30 < dist(move_island) or (len(enemy_drones) > 8 and attack_drones < 2)) and len(
                        islands) > 3:
                    move = move_drone
                    attack_drones += 1
